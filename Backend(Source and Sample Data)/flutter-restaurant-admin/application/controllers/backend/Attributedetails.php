...
class Attributedetails extends BE_Controller {
	 ...
	 */
	function index($id = 0, $product_id=0) {
		...
		// load index logic
		parent::att_detail_index($id,$product_id);
	}

	/**
	 * Searches for the first match.
	 */
	function search( $id = 0, $product_id=0 ) {

		$product_id = $this->get_data( 'product_id' );
		$id = $this->get_data('header_id');
		...
		$this->data['action_title'] = array( 
			...
			array( 'url' => '/index/'. $id .'/' . $product_id, 'label' => get_msg( 'list_attribute_detail' ), 'mod_name' => "attributedetails"),
		...
		// load add list
		parent::att_detail_search( $id,$product_id );
	}
