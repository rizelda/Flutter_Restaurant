...
class PS_Model extends CI_Model {
  ...
  */
	 ...
	 * @param      boolean  $offset  The offset
	 */
	function get_all_not($discount_id, $limit = false, $offset = false ) {
		// where clause
		//$this->custom_conds();
	 ...
	 * @param      boolean  $offset  The offset
	 */
	function get_all_delete_products($conds, $limit = false, $offset = false ) {
		// where clause
	...
	*/
	 ...
	 * @return     <type>  ( description_of_the_return_value )
	 */
	function get_shop_by_touch_userid( $conds = array(), $limit = false, $offset = false )
	{
		
		$this->db->select('rt_shops.*'); 
		$this->db->from('rt_shops');
		$this->db->join('rt_touches', 'rt_touches.type_id = rt_shops.id');
		...
		if($conds['order_by_type'] == "") {
			$this->db->order_by("count(DISTINCT rt_touches.id)", "DESC");
		} else {
			$this->db->order_by("count(DISTINCT rt_touches.id)", $conds['order_by_type']);
		}
	...
	function get_all_product( $conds = array() )
	{
		...
		if($conds['prd_ids_from_dis_other'] != "") {
			...
			if($conds['prd_ids_from_dis'] != "") {
				...
				$sql = "SELECT *  FROM rt_products WHERE id NOT IN(". $conds['prd_ids_from_dis_other'] .")  ORDER BY CASE WHEN id in (". $conds['prd_ids_from_dis'] .") then -1 else id end,id, is_discount asc";

			} else {

				//Disocunt not yet selected the product so no need ordering
				$sql = "SELECT *  FROM rt_products WHERE id NOT IN(". $conds['prd_ids_from_dis_other'] .")";
			...
			if($conds['prd_ids_from_dis'] != "") {
				...
				$sql = "SELECT *  FROM rt_products ORDER BY CASE WHEN id in (". $conds['prd_ids_from_dis'] .") then -1 else id end,id, is_discount asc";

			} else {
				//Disocunt not yet selected the product so no need ordering
				$sql = "SELECT *  FROM rt_products ";
			}
	...
	*/
	...
	/**
	Returns recent comment
	*/
	function get_rec_comment( $conds = array() )
	{
	
		$this->db->select('rt_comments_header.*');    
  		$this->db->from('rt_comments_header');
  		$this->db->limit(2);
	...
	/**
	Returns recent transaction
	*/
	function get_rec_transaction( $conds = array() )
	{
	...
	function get_all_zone_id( $conds = array()) {
		...
		$sql = "SELECT `rt_zones`.* FROM `rt_zones` WHERE `rt_zones`.`id` NOT in (". $conds['shi_zone_id'] .")";
	...
	function get_all_result_zone_id( $conds = array()) {
		...
		$sql = "SELECT `rt_zones`.* FROM `rt_zones` WHERE `rt_zones`.`id` in (". $conds['result_zone_id'] .")";
