...
class PS_Delete {
	...
	{		
		...
		if ( $enable_trigger ) {
		// if execute_trigger is enable, trigger to delete category related data
			if ( ! $this->delete_category_trigger( $category_id )) {
			// if error in deleteing category and category related data
	...
	function delete_category_trigger( $cat_id )
	{
		...
		if ( !empty( $products )) {
		// if the product list not empty
			
			// loop all the category
	...
	*/
	function delete_product_trigger( $product_id )
	{
		...
		$header_id = $this->CI->Commentheader->get_one_by($conds)->id;
		$conds_header =  array( 'header_id' => $header_id );
		...
		if ( !$this->CI->Commentdetail->delete_by( $conds_header )) {
	...
	*/
	...
	{		
		...
		if ( $enable_trigger ) {
		// if execute_trigger is enable, trigger to delete category related data
			if ( ! $this->delete_attribute_trigger( $attribute_id )) {
			// if error in deleteing category and category related data
	...
	function delete_attribute_trigger( $attribute_id )
	{
		//sub category condition
		$att_details = $this->CI->Attributedetail->get_all_by( array( 'header_id' => $attribute_id ))->result();
		foreach ( $att_details as $att ) {
			
			if ( !$this->delete_attdetail( $att->id, $enable_trigger )) {
			// if error in deleting category,
	 ...
	 * @param      <type>  $id     The identifier
	 */
	function delete_restaurant_branch( $id, $enable_trigger = false )
	{		
		if ( ! $this->CI->Restaurant_branch->delete( $id )) {
		// if there is an error in deleting Coupon,
	 ...
	 * @param      <type>  $id     The identifier
	 */
	function delete_delivery_status ( $id, $enable_trigger = false )
	{		
		if ( ! $this->CI->Deliverystatus->delete( $id )) {
	 ...
	 * @param      <type>  $id     The identifier
	 */
	function delete_unit ( $id, $enable_trigger = false )
	{		
		if ( ! $this->CI->Unit->delete( $id )) {
		// if there is an error in deleting unit,
