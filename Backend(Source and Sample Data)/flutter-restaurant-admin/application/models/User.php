...
class User extends PS_Model {
	...
	function custom_conds( $conds = array())
	{
		...
		// searchterm
		if(isset( $conds['searchterm'] ) || isset($conds['deliboy_status'])) {
			if ($conds['searchterm'] == "" && $conds['deliboy_status'] != "") {
				$this->db->where( 'status', $conds['deliboy_status'] );
			} else if($conds['searchterm'] != "" && $conds['deliboy_status'] != "") {
				$this->db->where( 'status', $conds['deliboy_status'] );
				$this->db->group_start();
				$this->db->like( 'user_name', $conds['searchterm'] );
				$this->db->or_like( 'user_email', $conds['searchterm'] );
				$this->db->group_end();
			} else if ($conds['searchterm'] !="") {
				$this->db->group_start();
				$this->db->like( 'user_name', $conds['searchterm'] );
				$this->db->or_like( 'user_email', $conds['searchterm'] );
				$this->db->group_end();
		...
		// verify_types condition
		if ( isset( $conds['verify_types'] )) {
			$this->db->where( 'verify_types', $conds['verify_types'] );
		}
		...
		// user_lat condition
		if ( isset( $conds['user_lat'] )) {
			$this->db->where( 'user_lat', $conds['user_lat'] );
		}

		// user_lng condition
		if ( isset( $conds['user_lng'] )) {
			$this->db->where( 'user_lng', $conds['user_lng'] );
		}
